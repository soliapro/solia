...
// Dossiers à ignorer dans demos/
const IGNORE = new Set(['dashboard', 'formulaire']);

function escapeJson(str) {
  return JSON.stringify(str).slice(1, -1); // échappe pour embedding dans JS
}
...
function generate() {
  ...
  // Injecter les données en JSON dans le HTML
...
<body>
  ...
  <main class="main">
    <div class="header">
      <h1>Prospection</h1>
      <p>${total} prospects &middot; ${withPhone} avec t&eacute;l&eacute;phone &middot; ${withPage} page(s) en ligne</p>
    </div>
  ...
  <script>
    ...
    function render() {
      ...
      // Update stat
      document.getElementById('stat-contacted').textContent =
        Object.keys(tracking).length;

      if (!filtered.length) {
        tbody.innerHTML = '<tr><td colspan="5" class="empty-row">Aucun prospect trouv&eacute;</td></tr>';
        return;
      }
      ...
      tbody.innerHTML = filtered.map(p => {
        ...
        const phoneRaw = p.telephone.replace(/\\s/g, '');
        ...
        return \`<tr data-slug="\${p.slug}">
          ...
          <td class="cell-phone">
            \${p.telephone ? '<a href="tel:' + esc(phoneRaw) + '">' + esc(p.telephone) + '</a>' : '<span style="color:var(--border)">—</span>'}
          </td>
          <td class="hide-mobile cell-avis">
            \${p.avis_note ? '<span class="star">&#9733;</span> ' + p.avis_note + '/5 <span style="color:var(--muted)">(' + p.avis_nb + ')</span>' : '<span style="color:var(--border)">—</span>'}
          </td>
          <td class="hide-mobile">
            \${p.has_page ? '<span class="badge badge-page">En ligne</span>' : '<span class="badge badge-no-page">Pas de page</span>'}
            \${t ? '<span class="badge badge-contacted" style="margin-left:4px">' + dateStr + '</span>' : ''}
          </td>
          <td class="actions">
            \${p.has_page
              ? '<a href="https://solia.me/formulaire/?prospect=' + p.slug + '" target="_blank" class="btn-sm btn-primary">Lien prospect</a>'
              : '<a href="https://solia.me/formulaire/?prospect=' + p.slug + '" target="_blank" class="btn-sm">Formulaire</a>'
            }
            \${t
              ? '<button class="btn-sm btn-done" onclick="unmark(\\'' + p.slug + '\\')">&#10003; ' + dateStr + '</button>'
